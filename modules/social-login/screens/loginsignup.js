...
const SocialButtonsView = props => <View>
    ...
    <GoogleSigninButton onPress={props.onGoogleConnect} size={GoogleSigninButton.Size.Wide} color={GoogleSigninButton.Color.Dark} disabled={props.loading} style={_styles.QWLfnNAQ} />
    {(Platform.OS === "ios" || appleAuthAndroid.isSupported) && <AppleButton onPress={props.onAppleConnect} buttonStyle={AppleButton.Style.WHITE_OUTLINE} buttonType={AppleButton.Type.SIGN_IN} style={_styles.ZKNDvwJy} />}
  </View>;

const onFacebookConnect = async dispatch => {
  try {
    const fb_result = await LoginManager.logInWithPermissions(["public_profile", "email"]);
